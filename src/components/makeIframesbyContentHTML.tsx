...
import { Nicovideo } from "./content/Nicovideo";
...
export const makeIframesbyContentHTML = (content, note) => {
  ...
  for (let i = 0; i < splitContent.length; i++) {
    ...
    if (tmpUrl.includes("youtube.com") || tmpUrl.includes("youtu.be/")) {
      ...
      const { out_iframe1, out_appleMusicIdText1, out_linkc } =
        AppleMusic(tmpUrl);
      ...
      tmpUrl.includes("open.spotify.com") &&
      !tmpUrl.includes("playlist")
    ) {
    ...
    // todo nicovideo 再生できない
      ...
      tmpUrl.includes("www.nicovideo.jp/watch") ||
      tmpUrl.includes("nico.ms")
    ) {
      const tmpIframe = Nicovideo(tmpUrl);

      serviceText = "__niconico";
      serviceText =
        '<a href="' + tmpUrl + '" target="_blank">' + serviceText + "</a>";
    ...
    // http iframe
      ...
      if (
        ...
        const tmpIframe =
          ...
          httpLinkUrl1 +
          '" width="300" height="150" frameborder="0" scrolling="no" ></iframe>(c)';
        iframe1 = tmpIframe;
        // URL 非表示
        ...
        const tmpIframe =
          ...
          httpLinkUrl1 +
          '" target="_blank">' +
          httpLinkUrl1.replace("http", "(c)_ttp") +
          "</a>";
        iframe1 = tmpIframe;
        // URL 非表示
        content = content.replace(httpLinkUrl1, "");
  ...
  return content + iframe1 + serviceText;
};
